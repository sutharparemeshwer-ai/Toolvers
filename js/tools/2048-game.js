...
let boardEl, scoreEl, newGameBtn, tryAgainBtn, gameOverMsgEl;
let board = [];
let score = 0;
const GRID_SIZE = 4;
...
function moveLeft() {
    for (let r = 0; r < GRID_SIZE; r++) {
        const start = r * GRID_SIZE;
        const row = board.slice(start, start + GRID_SIZE);
        const newRow = operate(row);
        for (let c = 0; c < GRID_SIZE; c++) board[start + c] = newRow[c];
    }
}
...
function moveUp() {
    for (let c = 0; c < GRID_SIZE; c++) {
        ...
        for (let r = 0; r < GRID_SIZE; r++) board[r * GRID_SIZE + c] = newCol[r];
    }
}
...
function checkGameOver() {
    for (let i = 0; i < board.length; i++) {
        ...
        const c = i % GRID_SIZE;
        if (c < GRID_SIZE - 1 && board[i] === board[i + 1]) return;
        if (r < GRID_SIZE - 1 && board[i] === board[i + GRID_SIZE]) return;
    }
...
export function init() {
    ...
    if(tryAgainBtn) tryAgainBtn.addEventListener('click', setupGame);
